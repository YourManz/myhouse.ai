...
export function FloorPlanEditor({ width, height }: FloorPlanEditorProps) {
  ...
  const {
    floorPlan, activeFloor, activeFloorId, activeTool, selectedIds,
      ...
      floors: fp.floors.map(f => {
        ...
        for (const vIdx of room.vertices) {
          if (!movedSet.has(vIdx)) {
            movedSet.add(vIdx)
            newVertices[vIdx] = { x: newVertices[vIdx].x + dx, y: newVertices[vIdx].y + dy }
          }
        }
  ...
  return (
    <Stage
      ...
      onDragEnd={e => { setCanvasTransform({ x: e.target.x(), y: e.target.y() }) }}
      onWheel={handleWheel}
    ...
    >
      ...
      <RoomLayer
        ...
        onSelect={id => setSelectedIds([id], 'room')}
      ...
      <WallLayer
        ...
        activeTool={activeTool}
        selectedIds={selectedIds}
        onSelect={id => setSelectedIds([id], 'wall')}
        scale={floorPlan.scale}
      />
