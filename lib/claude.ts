...
const THREE_D_SPEC_SYSTEM = `You are a senior architectural designer specializing in 3D building design.
Given a floor plan JSON, propose a ThreeDSpec — 3D architectural decisions that will make this building visually interesting and stylistically coherent.
Respond with ONLY valid JSON — no markdown, no code fences.

For each cutout, ceiling override, and feature: include a short "rationale" explaining why it improves the design.
Set all "enabled" fields to true — the user may toggle them off.
Be bold and specific: name the pitch, the overhang, the exact rooms for vaulted ceilings.
Match the roof type and features to the architectural style.
...
- mediterranean: flat with parapet, courtyard-void if U-shape, terracotta
...
Respond with this exact JSON shape:
{
  "roof": {
    ...
    "subRoofs": null|[{"footprintRoomIds": string[], "type": string, "pitch": number}]
  },
  "cutouts": [{
    "id": string,
    "type": string,
    "targetFloor": number,
    "targetWallId": null|string,
...
Match materials to architectural style and room function.
Set all "enabled" to true.
Provide a short "rationale" per surface explaining the design choice.
...
export async function generateFloorPlan(prompt: string): Promise<AIFloorPlanResponse> {
  ...
  return JSON.parse(text) as AIFloorPlanResponse
}

export async function extract3DSpec(floorPlan: FloorPlan): Promise<ThreeDSpec> {
  const message = await client.messages.create({
    model: 'claude-sonnet-4-6',
    max_tokens: 4096,
...
Floor plan data:
${JSON.stringify({ title: floorPlan.title, floors: floorPlan.floors.map(f => ({ level: f.level, label: f.label, rooms: f.rooms.map(r => ({ id: r.id, type: r.type, label: r.label })) })) }, null, 2)}`,
    }],
  })

  const text = message.content[0].type === 'text' ? message.content[0].text : ''
  return JSON.parse(text) as ThreeDSpec
}

export async function getMaterialPalette(floorPlan: FloorPlan): Promise<MaterialPalette> {
  const message = await client.messages.create({
    model: 'claude-sonnet-4-6',
    max_tokens: 2048,
    ...
    messages: [{
      role: 'user',
      content: `Architectural style: ${floorPlan.style ?? 'modern'}
Room types present: ${[...new Set(floorPlan.floors.flatMap(f => f.rooms.map(r => r.type)))].join(', ')}
...
Roof type: ${floorPlan.threeDSpec?.roof.type ?? 'unknown'}`,
  ...
  const text = message.content[0].type === 'text' ? message.content[0].text : ''
  return JSON.parse(text) as MaterialPalette
}
